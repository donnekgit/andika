...
// truncate the array of vipande letters to the total number of vipande in the stanza
// NOTE: if you put $vipande= at the front of the line, $vipande will contain the cut pieces.
// print_r($vipande);
...
if ($output=="pdf")
{
    include("tex/pdf_open.php");  // points to a collection of tex stuff.
}
elseif ($output=="txt")
{
...
foreach ($poemlines as $poemline)
{
    ...
    {
        $stanza_contents.=trim($poemline)." * ";
    }
    else // it's a blank line preceding a new stanza, so print out the previous stanza 
    {
        echo "\n".$stanza_no."\n";

        $this_stanza=array_filter(array_map('trim', explode("*", $stanza_contents)));
//         print_r($this_stanza);
