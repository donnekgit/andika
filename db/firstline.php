...
/*
This script takes words from the db table and outputs the lines centred in star format, with each line's transcription included as a footnote.  It is designed for poems like the Burda, where the first line of a stanza is in Arabic, with the subsequent lines in Swahili.  Set the colours for the lines at the top of the script. Run as:
php db/firstline.php <name of the poem>
*/
...
$columns="cl";  // Set up the columns for the main table which will hold the text.
$colour1="blue";  // Colour for the first line.
//$colour2="Green4";  // Colour for the other lines.
$colour2="mygreen";  // Colour for the other lines.
...
fwrite($fp, "\\centering\\textcolor{red}{\\textarabic{بِسْمِآللّٰهِ الرَّحْمٰن الرَّحِْمِ}}\n\n\n");
...
while ($row=pg_fetch_object($sql))
{
    ...
    while ($row_loc=pg_fetch_object($sql_loc))
    {
        ...
        if($double==1)  // We have two kipande on the line, so print them.
        {
            ...
            {
               fwrite($fp, "\\footnote{\\Tr{".$a_close." * ".$b_close."}} ");
            ...
            {
                fwrite($fp, "\\footnote{\\Tr{".$a_close." * ".$b_close."}} ");
                fwrite($fp, "\\textcolor{{$colour2}}{\\textarabic{".$a_arabic." * ".$b_arabic."}} &");
            }
            fwrite($fp, " \\\\* \n");
